...
def test_project_brief_payload_adds_reference_product_intelligence_to_memory() -> None:
    image_analysis = analyze_reference_images(
        ReferenceImageInput(
            images=[
                ReferenceImageItem(
    ...
    assert "Documents" in payload["reference_product_analysis"][
...
def test_project_brief_payload_without_reference_image_keeps_existing_memory_shape() -> None:
    payload = project_service_module._project_brief_payload(
...
def test_reference_product_intelligence_does_not_override_user_product_domain() -> None:
    image_analysis = analyze_reference_images(
        ReferenceImageInput(
            images=[
                ReferenceImageItem(
    ...
    assert payload["product_memory"]["domain"] == "fitness"
    assert payload["reference_product_analysis"]["product_type"] == "SaaS Workspace"
    assert payload["product_brief"]["product_name"] == "Trainer Fitness App"
